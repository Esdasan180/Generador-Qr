...
with st.sidebar.expander("Ver cuentas"):
    ...
    st.markdown("[Github](https://github.com/Esdasan180)")
    st.markdown("[Linkedin](https://www.linkedin.com/in/estebansantillansuarez/)")

st.sidebar.write(
    """He realizado el presente trabajo con el fin de usar streamlit 
...
st.markdown(':large_yellow_circle: Para tener una imagen o gif de fondo puedes arrastrar el \
            archivo hacia la nube o seleccionas donde dice "Browse files" \
...
with intro2:
    ...
    url = st.text_input(":large_purple_circle: introduce el enlace a la :red[imagen] o :red[gif] que quieres \
                        tener de fondo",
                          ...
                          help = """Si al insertar un enlace no se genera una imagen chequea que tenga un formato compatible (JPG, PNG, GIF),
                            te muestro algunos ejemplos: 
                            ...
                            """)
    texto = str(url).lower()
    if ('png' in texto) or ('jpg' in texto) or ('gif' in texto):
        pass
    else:
        st.markdown(
                f"""<p style='font-size: 13px; color: #808080;text-align: center;'>enlace no compatible</p>""",  
